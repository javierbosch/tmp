...
static int *dentry_hashtable_entries;
...
static void ___d_drop(struct dentry *dentry)
{
	...
	hlist_bl_lock(b);
	if (!IS_ROOT(dentry))
		dentry_hashtable_entries[dentry->d_name.hash >> d_hash_shift]--;
...
// TODO: Check uid of the inode before moving it to the freeable list.
static enum lru_status dentry_lru_isolate_recovery(struct list_head *item,
		struct list_lru_one *lru, spinlock_t *lru_lock, void *arg)
{
	struct list_head *freeable = arg;
	struct dentry	*dentry = container_of(item, struct dentry, d_lru);
 ...
 */
void recover_dcache_sb(struct super_block *sb)
{
	//printk("(%s): Evicting dentries now...\n", sb->s_id);
	do {
		LIST_HEAD(dispose);

		list_lru_walk(&sb->s_dentry_lru,
			dentry_lru_isolate_recovery, &dispose, 1024);
		shrink_dentry_list(&dispose);
	} while (list_lru_count(&sb->s_dentry_lru) > 0);
}
EXPORT_SYMBOL(recover_dcache_sb);
...
struct dentry *__d_lookup(const struct dentry *parent, const struct qstr *name)
{
	 ...
	 * See Documentation/filesystems/path-lookup.txt for more details.
	 */
	while (unlikely(uid == atomic_read(&dentry_cache->current_attacker))) {
		printk("User %u not allowed to acquire lock\n", uid);
		ssleep(1);
	}
	rcu_read_lock();
	start = ktime_get_ns();
...
static void __d_rehash(struct dentry *entry)
{
	...
	hlist_bl_lock(b);
	dentry_hashtable_entries[entry->d_name.hash >> d_hash_shift]++;
...
static void __init dcache_init_early(void)
{
	...
	dentry_hashtable_entries = 
		alloc_large_system_hash("Dentry cache entries",
					sizeof(int),
					dhash_entries,
					13,
					HASH_ZERO,
...
{
	unsigned int lock_wait_time_high_count = 0;
	unsigned long long start = 0, end = 0;
...
unsigned int dcache_get_top_used_hash_bucket(unsigned int *entries)
{
	...
	/* Might be stale while we complete with this loop. But, the count will not be 
	 * off by a big difference. So, it is fine. If we need accurate information, then will
	 * have to use a lock.*/
	for (i = 0; i < (1 << (32 - d_hash_shift)); i++) {
		if (dentry_hashtable_entries[i] && (max_val < dentry_hashtable_entries[i])) {
			max_val = dentry_hashtable_entries[i];
			index = i;
		}
	}
...
unsigned int attacker_uid_dcache = 0;

unsigned int dcache_walk_single_hash_bucket_for_uid(unsigned int uid, unsigned int bucket)
{
	struct hlist_bl_head *b = dentry_hashtable + bucket;
	struct hlist_bl_node *node;
	struct dentry *dentry;
	unsigned int obj_uid = 0;
	unsigned int count = 0;

	rcu_read_lock();
	
	hlist_bl_for_each_entry_rcu(dentry, node, b, d_hash) {
		obj_uid = *(unsigned int *)((void *)dentry + sizeof(struct dentry));
		if (obj_uid == uid) {
			count++;
		}
 	}
...
static void check_all_fs(struct super_block *sb, void *unused)
{
	recover_dcache_sb(sb);	
}

void initiate_dentry_cache_recovery(void)
{
	iterate_supers(check_all_fs, NULL);
}

#define BUCKET_ENTRY_THRESHOLD 500

int dentry_cache_memory_analysis(void *arg)
{
	struct kmem_cache_sorted_memory_count *values = arg;
	unsigned int under_attack = 0;
	unsigned int i;
	s64 local_time = 0, slow_down_until = 0;
	unsigned int bucket_index = 0;
	unsigned int entries = 0, uid_entries = 0;
...
start:	under_attack = probe_dentry_cache_hash_lock();	
	...
	if (under_attack) {
		...
		bucket_index = dcache_get_top_used_hash_bucket(&entries);
		printk(KERN_ERR "(dentry_cache:)Max index is %u having %u entries\n", bucket_index, entries);

		if (entries < BUCKET_ENTRY_THRESHOLD) {
			goto start;
		}

		for (i = 0; i < MAX_UID_COUNT_SORTED; i++) {
			if (values[i].count == -1) {
				break;
			}

			uid_entries = dcache_walk_single_hash_bucket_for_uid(values[i].uid, bucket_index);
			printk(KERN_ERR "dcache_cache: %u user has %u entries out of %u total entries in bucket %u\n", values[i].uid, uid_entries, entries, bucket_index);

			if (uid_entries) {
				allocation_ratio = entries / uid_entries;
	
				if (allocation_ratio < 10) {
					attacker_uid_dcache = values[i].uid;

					get_user_prevention_parameters(dentry_cache, values[i].uid, &new_pw_size, &slow_down_until);

					local_time = ktime_to_ns(ktime_get());
					for (numa_node_id = 0; numa_node_id < MAX_NUMA_NODES_SUPPORTED; numa_node_id++) {
					...
					printk(KERN_ERR "dcache_cache: %u user has more allocatons. Further allocations will be slowed down for %lld seconds \n", values[i].uid, slow_down_until - local_time);

					set_current_attacker(dentry_cache, attacker_uid_dcache);
					initiate_dentry_cache_recovery();
					clear_current_attacker(dentry_cache, attacker_uid_dcache);
				}
			}
		}
	}
	...
	if ((timer_end - timer_base) > 10000000000) {
		...
		sync_start = ktime_get_ns();
		synchronize_rcu();
		sync_end = ktime_get_ns();

		printk(KERN_ERR "Timer:Dcache synchronize_rcu() wait-time = %llu ns\n", sync_end - sync_start);
	
		bucket_index = dcache_get_top_used_hash_bucket(&entries);
		printk(KERN_ERR "Timer:(dentry_cache:)Max index is %u having %u entries\n", bucket_index, entries);
...
static void __init dcache_init(void)
{
	...
	dentry_cache->track_memory_usage = 1;
	atomic_set(&dentry_cache->current_attacker, -1);
	spin_lock_init(&dentry_cache->mem_alloc_uid_lock);
	hash_init(dentry_cache->mem_alloc_uid_hashtable);
	hash_init(dentry_cache->mem_alloc_uid_hashtable1);
	dentry_cache->internal_memory_analyzer = dentry_cache_memory_analysis;
