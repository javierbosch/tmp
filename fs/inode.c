...
{
	unsigned int h_hash_shift, h_hash_mask;
	...
	inode_hash_tables_entries =
		alloc_large_system_hash("Inode-hash-entries",
					sizeof(int),
					ihash_entries,
					14,
					HASH_ZERO,
...
again:
	...
	list_for_each_entry_safe(inode, next, &sb->s_inodes, i_sb_list) {
		...
		/*
		 * We can have a ton of inodes to evict at recovery time given
		 * enough memory, check to see if we need to go to sleep for a
		 * bit so we don't livelock.
		 */
		if (need_resched()) {
	...
	return inodes_evicted;
}
EXPORT_SYMBOL_GPL(evict_inodes_by_uid);
...
unsigned int probe_inode_cache_hash_lock(void)
{
...
EXPORT_SYMBOL_GPL(probe_inode_cache_hash_lock);

unsigned int get_top_used_hash_bucket(unsigned int *entries)
{
	unsigned int i = 0;
	int max_val = 0;
	...
	for (i = 0; i < (1 << i_hash_shift); i++) {
		if ((inode_hash_tables_entries[i]) && (max_val < inode_hash_tables_entries[i])) {
			max_val = inode_hash_tables_entries[i];
			index = i;
		}
	}
	spin_unlock(&inode_hash_lock);
...
EXPORT_SYMBOL_GPL(get_top_used_hash_bucket);

unsigned int walk_single_hash_bucket_for_uid(unsigned int uid, unsigned int bucket, unsigned int (*f)(struct inode *))
{
	struct hlist_head *head = inode_hashtable + bucket;
	struct inode *inode;
	int count = 0;
	unsigned int inode_uid;
...
EXPORT_SYMBOL_GPL(walk_single_hash_bucket_for_uid);
...
void __init inode_init(void)
{
	...
	atomic_set(&inode_cache_attacker, -1);
	/* inode slab cache */
